...
from vqa import run_vqa  # VQA helper
...
# Endpoint to handle question submission (text + articles flow)
...
def ask():
    ...
    first_url, search_term = pmc.question_formation(
        question, model, temperature, api_key
    )
    idlist = pmc.idlist_confirm(
        first_url, question, article_number, model, temperature, api_key, search_term
    )
    print("confirmed url")
    ...
    summaries, citations = pool_summary.spawn(
        question, urls, model, citation_format, temperature, api_key
    )
    print("generating response")
    response = csg.generate_summary(
        summaries, question, model, citation_format, citations, temperature, api_key
    )
...
# VQA endpoint (image + question)
@app.route("/vqa", methods=["POST"])
def vqa_endpoint():
    # Expecting a multipart/form-data request with:
    ...
    # Let vqa.run_vqa handle reading bytes + OpenAI→Groq fallback
    answer = run_vqa(image_file, question)
