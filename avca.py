...
Public entry points:
- validate_articles(query, articles, min_score_to_keep=0.15)
...
UNKNOWN_JOURNAL_BASE = 0.4  # slightly more generous for unknown journals
...
def reputation_score(article: Article) -> float:
    ...
    Rules:
        ...
        - Otherwise (e.g., preprint/unknown) -> UNKNOWN_JOURNAL_BASE
    """
...
def validate_articles(
    ...
    Args:
        ...
        min_score_to_keep: Minimum validation_score required for an article
                           to be retained. This is intentionally low by
                           default (0.15) to accommodate noisy or
                           metadata-poor corpora.
    ...
    for a in articles:
        ...
        # Label bands for interpretability; filtering is controlled by min_score_to_keep
        if score >= 0.75:
    ...
    # More tolerant filtering: retain anything above min_score_to_keep
    filtered = [a for a in validated if a.validation_score >= min_score_to_keep]
    logger.info(
        "validate_articles returning %d/%d articles (min_score_to_keep=%.3f).",
        len(filtered),
        len(validated),
        min_score_to_keep,
    )
