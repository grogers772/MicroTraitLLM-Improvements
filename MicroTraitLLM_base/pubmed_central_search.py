...
def question_formation(prompt,model,temperature,api_key,ncbi_api_key):
    ...
    if model == "ChatGPT-4o-mini":
        ...
        if re_search_term is not None:
            ...
            link = f'https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pmc&retmode=json&retmax=20&sort=relevance&term={search_term}+AND+open+access[filter]&api_key={ncbi_api_key}'
        else:
            search_term = None
            link = None
...
def idlist_confirm(inp,prompt,article_number,model,temperature,api_key,search_term,ncbi_api_key):
    ...
    while data is None or int(b) == 0:
        ...
        if model == "ChatGPT-4o-mini":
            ...
            if re_search_term is not None:
                search_term = re_search_term.group()
                link =  link = f'https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pmc&retmode=json&retmax=20&sort=relevance&term={search_term}+AND+open+access[filter]&api_key={ncbi_api_key}'
            ...
            if re_search_term is not None:
                search_term = re_search_term.group()
                link = f'https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pmc&retmode=json&retmax=20&sort=relevance&term={search_term}&api_key=83e5ed87f0d4e27474b0681f748747f6e908'
                data = call_api.call_api(link)
                data = json.loads(data)
                b = data.get('esearchresult',{}).get('count')
            else:
